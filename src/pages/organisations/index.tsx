...
type ColumnType = ColumnDef<OrganisationInterface, unknown>;

interface OrganisationListPageProps {
...
export function OrganisationListPage(props: OrganisationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<OrganisationInterface>>(
    () => `/organisations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: OrganisationInterface) => {
    if (hasAccess('organisation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/organisations/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/organisations/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('organisation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/organisations/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Box p={4} rounded="md" shadow="none">
      ...
      >
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="organisation" />
        </Flex>

        {hasAccess('organisation', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/organisations/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
