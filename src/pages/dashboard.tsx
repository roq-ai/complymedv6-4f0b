...
import { getAuditReports } from 'apiSdk/audit-reports';
import { AuditReportInterface } from 'interfaces/audit-report';

import { OrganisationListPage as ShowCaseList } from 'pages/organisations';
...
function DashboardPage() {
  ...
  const organisationStat = useSWR<PaginatedInterface<OrganisationInterface>>(
    () => `/organisations`,
    () => getOrganisations({ limit: 1, offset: 0 }),
  );

  const auditReportStat = useSWR<PaginatedInterface<AuditReportInterface>>(
    () => `/audit-reports`,
    () => getAuditReports({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Organisations',
      result: organisationStat?.data?.totalCount,
      isLoading: organisationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Audit Reports',
      result: auditReportStat?.data?.totalCount,
      isLoading: auditReportStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`organisations`} passHref legacyBehavior>
                  <Button
